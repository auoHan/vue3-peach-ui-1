<template>
  <button class="peach-button" :class="classes" :disabled="disabled">
    <span v-if="loading" class="peach-loadingIndicator"></span>
    <slot />
  </button>
</template>
...
export const classes = computed(() => {
  return {
    [`peach-theme-${theme}`]: theme,
    [`peach-size-${size}`]: size,
    [`peach-level-${level}`]: level,
  };
});
</script>
<style lang="scss">
